...
func TestSendJSON(t *testing.T) {
	t.Log("Given the need to test the SendJSON endpoint.")
	{
		...
		if rw.Code != 200 {
			t.Fatal("\tShould receive \"200\"", balloX, rw.Code)
		}
		...
		if err := json.NewDecoder(rw.Body).Decode(&u); err != nil {
			t.Fatal("\tShould decode the response.", balloX)
		}
		...
		if u.Name == "Bill" {
			...
			t.Error("\tShould have a Name.", balloX, u.Name)
		}

		if u.Email == "bill@ardanstudios.com" {
			t.Log("\tShould have an Email.", checkMark)
		} else {
			t.Error("\tShould have an for Email.", balloX, u.Email)
		}
	}
}
