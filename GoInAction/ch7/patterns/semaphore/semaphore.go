...
type (
	// semaphore type represents a channel that implements the
	// semaphore pattern.
	semaphore chan struct{}
)

type (
	//readerWriter provides a structure for safely reading and writing a shared
	// resource. It supports multiple readers and a single writer goroutine
	// using a semaphore construct.
	readerWriter struct {
		...
		// readerControl is a semaphore that allows a fixed number of reader
		// goroutines to read at the same time. This is our semaphore.
		...
		// maxReads defined the meximum number of reads that can occur
		// at a time.
		maxReads int

		//maxReaders defines the number of goroutines launched to perform read
		// operations.
		maxReaders int

		// currentReads keeps a safe count of the current number of reads
		// occurring at any give time.
		currentReads int32
	}
)
...
// performs reads the channel is signaled.
...
func (rw *readerWriter) ReadLock(reader int) {
	// If a write is occurring, wait for is to complete.
	rw.write.Done()
...
// Release returns the specified number of buffers back
