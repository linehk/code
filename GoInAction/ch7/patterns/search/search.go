...
// OnlyFrist is an option that will restrict the search session to just the
// first result.
func OnlyFrist(s *searchSession) { s.first = true }
	...
	for search := 0; search < len(session.searchers); search++ {
		...
		if session.first && search > 0 {
			go func() {
				...
				log.Printf("search : Submit : Info : Result Discarded : "+
					"Results[%d]\n", len(r))
			}()
			continue
		}
