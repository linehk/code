...
// no encoding of JPEG
func cut(original image.Image, db *DB,
	tileSize, x1, y1, x2, y2 int) <-chan image.Image {
		...
		for y := y1; y < y2; y = y + tileSize {
			for x := x1; x < x2; x = x + tileSize {
				...
				if err == nil {
					...
					fmt.Println("error opening file when creating mosaic:",
						nearest)
				}
				file.Close()
			}
...
func combine(r image.Rectangle,
	...
	go func() {
		...
		copy := func(dst draw.Image, r image.Rectangle, src image.Image,
			sp image.Point) {
...
func mosaic(w http.ResponseWriter, r *http.Request) {
	...
	r.ParseMultipartForm(10485760) // max body in memory in 10MB
