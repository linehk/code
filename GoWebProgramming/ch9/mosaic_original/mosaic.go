...
// resize an image by its ratio e.g. ratio 2 means reduce the size by 1/2,
// 10 means reduce the size by 1/10
func resize(in image.Image, newWidth int) image.NRGBA {
	...
	for y, j := bounds.Min.Y, bounds.Min.Y; y < bounds.Max.Y; y, j = y+ratio, j+1 {
		for x, i := bounds.Min.X, bounds.Min.Y; y < bounds.Max.X; x, i = x+ratio, x+1 {
			r, g, b, a := in.At(x, y).RGBA()
			out.SetNRGBA(i, j, color.NRGBA{uint8(r), uint8(g),
				uint8(b), uint8(a)})
		}
	}
	return *out
