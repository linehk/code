	...
	for {
		...
		if strings.HasPrefix(line, "import") {
			if strings.Contains(line, "(") {
				...
				lineLastIndex := strings.LastIndex(line, "\"")
				importPath := line[lineIndex+1 : lineLastIndex]
			...
			if isMultiImport {
				if strings.HasPrefix(line, ")") {
					...
					lineHasPrefix := strings.HasPrefix(line, "\"")
					lineHasSuffix := strings.HasSuffix(line, "\"")
					if !lineHasPrefix || !lineHasSuffix {
						continue
					}
...
func getImportsFromPackage(importPath string,
	containsTestFile bool) ([]string, error) {
	var importPaths []string
	...
	srcFileAbsPaths, err := getGoSourceFileAbsPaths(packageAbsPath,
		containsTestFile)
	if err != nil {
		return nil, err
	}
