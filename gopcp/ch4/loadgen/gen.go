...
func (gen *myGenerator) init() error {
	...
	// 载荷的并发量 约等于 载荷的响应超时时间 / 载荷的发送间隔时间
...
func (gen *myGenerator) prepareToStop(ctxError error) {
	...
	atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_STARTED,
		lib.STATUS_STOPPING)
...
func (gen *myGenerator) Start() bool {
	...
	if !atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_ORIGINAL,
		lib.STATUS_STARTING) {
		if !atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_STOPPED,
			lib.STATUS_STARTING) {
			return false
		}
	}
	...
	// 初始化上下文和取消函数。
	gen.ctx, gen.cancelFunc = context.WithTimeout(context.Background(),
		gen.durationNS)
...
func (gen *myGenerator) Stop() bool {
	if !atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_STARTED,
		lib.STATUS_STOPPING) {
		return false
	}
	gen.cancelFunc()
