...
func TestCmapNew(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("No error when new a concurrent map with concurrency %d, "+
			"but should not be the case!", concurrency)
	}
	concurrency = MAX_CONCURRENCY + 1
	cm, err = NewConcurrentMap(concurrency, pairRedistributor)
	if err == nil {
		t.Fatalf("No error when new a concurrent map with concurrency %d, "+
			"but should not be case!", concurrency)
	}
	concurrency = 16
	cm, err = NewConcurrentMap(concurrency, pairRedistributor)
	if err != nil {
		t.Fatalf("An error occurs when new a concurrent map: %s "+
			"concurrency: %d, pairRedistributor: %#v", err, concurrency,
			pairRedistributor)
	}
	if cm == nil {
		t.Fatalf("Couldn't a new concurrent map! (concurrency: %d, "+
			"pairRedistributor: %#v)", concurrency, pairRedistributor)
	}
...
func TestCmapPut(t *testing.T) {
	...
	for _, p := range testCases {
		...
		if err != nil {
			t.Fatalf("An error occurs when putting a key-element to the "+
				"cmap: %s (key: %s, element: %#v)", err, key, element)
		}
		if !ok {
			t.Fatalf("Couldn't put key-element to the cmap! (key: %s, "+
				"element: %#v)", key, element)
		}
		...
		ok, err = cm.Put(key, element)
		if err != nil {
			t.Fatalf("An error occurs when putting a repeated key-element "+
				"to the cmap! %s (key: %s, element: %#v)", err, key, element)
		}
		if ok {
			t.Fatalf("Couldn't put key-element to the cmap! (key: %s, "+
				"element: %#v)", key, element)
...
func TestCmapPutInParallel(t *testing.T) {
	...
	testingFunc := func(key string, element interface{},
		t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			t.Parallel()
			ok, err := cm.Put(key, element)
			if err != nil {
				t.Fatalf("An error occurs when putting a key-element to "+
					"the cmap: %s (key: %s, element: %#v)", err, key, element)
			}
			if !ok {
				t.Fatalf("Couldn't put key-element to the cmap! (key: %s, "+
					"element: %#v)", key, element)
			...
			ok, err = cm.Put(key, element)
			if err != nil {
				t.Fatalf("An error occurs when putting a repeated "+
					"key-element to the cmap! %s (key: %s, element: %#v)",
					err, key, element)
			}
			if ok {
				t.Fatalf("Couldn't put key-element to the cmap! (key: %s, "+
	...
	if cm.Len() != uint64(number) {
		t.Fatalf("Inconsistent size: expected: %d, actual: %d", number,
			cm.Len())
	}
}
...
func TestCmapDelete(t *testing.T) {
	...
	for _, p := range testCases {
		...
		if !done {
			t.Fatalf("Couldn't delete a key-element from cmap! (key: %s, "+
				"element: %#v)", p.Key(), p.Element())
		}
		...
		if done {
			t.Fatalf("Couldn't delete a key-element from cmap again! (key: "+
				"%s, element: %#v)", p.Key(), p.Element())
		}
		if count > 0 {
			count--
		}
	...
	if cm.Len() != 0 {
		t.Fatalf("Inconsistent size: expected: %d, actual: %d", 0, cm.Len())
	}
}
...
func TestCmapDeleteInParallel(t *testing.T) {
	...
	testingFunc := func(key string, element interface{},
		...
		return func(t *testing.T) {
			...
			if !done {
				t.Fatalf("Couldn't delete a key-element from cmap! (key: "+
					"%s, element: %#v)", key, element)
			}
			actualElement := cm.Get(key)
			if actualElement != nil {
				t.Fatalf("Inconsistent key-element: expected: %#v, actual: %#v",
			...
			if done {
				t.Fatalf("Couldn't delete a key-element from cmap again! "+
					"(key: %s, element: %#v)", key, element)
			}
		}
	}
	...
	t.Run("All in parallel", func(t *testing.T) {
		t.Run("Put1", func(t *testing.T) {
			...
			for _, p := range testCases1 {
				...
				if err != nil {
					t.Fatalf("An error occurs when putting a key-element "+
						"to the cmap: %s (key: %s, element: %#v)", err, p.Key(),
						p.Element())
				}
			}
		})
		...
		t.Run("Get1", func(t *testing.T) {
			...
			for _, p := range testCases1 {
				...
				if actualElement != p.Element() {
					t.Fatalf("Inconsistent element: expected: %#v, actual: "+
						"%#v", p.Element(), actualElement)
				}
			}
		})
