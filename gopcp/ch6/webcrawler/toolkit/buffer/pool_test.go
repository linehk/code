...
func TestPoolNew(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("An error occurs when new a buffer pool: %s "+
			"(bufferCap: %d, maxBufferNumber: %d)", err, bufferCap,
			maxBufferNumber)
	}
	if pool == nil {
...
func TestPoolPut(t *testing.T) {
	...
	for _, datum = range data {
		err := pool.Put(datum)
		if err != nil {
			t.Fatalf("An error occurs when putting a datum to the buffer "+
				"pool: %s (datum: %d)", err, datum)
		}
		count++
		...
		if pool.BufferNumber() != expectedBufferNumber {
			t.Fatalf("Inconsistent buffer number: expected: %d, actual: %d "+
				"(count: %d)", expectedBufferNumber, pool.BufferNumber(), count)
		}
	}
	datum = dataLen
	select {
	...
	err = pool.Put(datum)
	if err == nil {
		t.Fatalf("It still can put datum to the closed buffer pool! "+
			"(datum: %d)", datum)
	}
}
...
func TestPoolPutInParallel(t *testing.T) {
	...
	testingFunc := func(datum interface{}, t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			...
			err := pool.Put(datum)
			if err != nil {
				t.Fatalf("An error occurs when putting a datum to the "+
					"buffer pool: %s (datum: %d)", err, datum)
			}
	...
	case err := <-addExtraDatum(pool, datum):
		if err != nil {
			...
			t.Fatal("It still can put a datum to the full buffer pool1")
		}
	case <-time.After(time.Millisecond):
		t.Logf("Timeout! Couldn't put data to the full buffer pool.")
	}
...
func TestPoolGet(t *testing.T) {
	...
	for i := uint32(0); i < dataLen; i++ {
		d, err := pool.Get()
		if err != nil {
			t.Fatalf("An error occurs when getting a datum from the "+
				"buffer pool: %s", err)
		}
		datum, ok = d.(uint32)
		if !ok {
		...
		t.Run("Put1", func(t *testing.T) {
			...
			for i := begin; i < end; i++ {
				if pool.Total() == uint64(dataLen) {
					...
					case err := <-addExtraDatum(pool, datum):
						if err != nil {
							t.Fatalf("An error occurs when putting a datum "+
								"to the buffer pool: %s (datum: %d)", err,
								datum)
						} else {
							t.Fatal("It still can put a datum to the full " +
								"buffer pool!")
						}
					case <-time.After(time.Millisecond):
						t.Logf("Timeout! Couldn't put data to the full " +
							"buffer pool.")
					}
					continue
				}
		...
		t.Run("Get1", func(t *testing.T) {
			...
			for i := uint32(0); i < max; i++ {
				if pool.Total() == 0 {
					...
					case err := <-getExtraDatum(pool):
						if err != nil {
							t.Fatalf("An error occurs when getting a datum "+
								"from the buffer pool: %s", err)
							// } else {
							// 	t.Fatal("It still can get a datum from the empty buffer pool!")
						}
					case <-time.After(time.Millisecond):
						t.Logf("Timeout! Couldn't get data from the empty " +
							"buffer pool.")
					}
					continue
				}
				...
				if d == nil && atomic.LoadUint32(&puttingCount) == 0 &&
					pool.Total() != 0 {
...
func TestPoolCloseInParallel(t *testing.T) {
	...
	t.Run("Close", func(t *testing.T) {
		...
		ok := pool.Close()
		if !ok {
			t.Fatalf("Couldn't close the buffer pool!")
		}
		if !pool.Closed() {
			t.Fatalf("Inconsistent buffer pool status: expected closed: %v, "+
				"actual closed: %v", true, pool.Closed())
		}
		ok = pool.Close()
		if ok {
