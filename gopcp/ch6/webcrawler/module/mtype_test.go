	...
	for _, mt := range legalTypes {
		...
		for _, m := range fakeModules {
			if m.ID() == matchedModule.ID() {
				if !CheckType(mt, m) {
					t.Fatalf("Inconsistent module type: expected: %T, "+
						"actual: %T", matchedModule, mt)
				}
			} else {
				if CheckType(mt, m) {
					t.Fatalf("The module type %T is not matched, but do not "+
						"be detected!", mt)
				}
			}
		}
...
func TestTypeGet(t *testing.T) {
	for _, mid := range legalMIDs {
		...
		if mt != expectedType {
			t.Fatalf("Inconsistent module type for letter: expected: %s, "+
				"actual: %s (MID: %s)", expectedType, mt, mid)
		}
	}
	for _, illegalMID := range illegalMIDs {
		...
		if string(mt) != "" {
			t.Fatalf("It still can obtain type %q from illegal MID %q!", mt,
				illegalMID)
		}
	}
}
...
func TestTypeGetLetter(t *testing.T) {
	for letter, mt := range legalLetterTypeMap {
		...
		if letter1 != letter {
			t.Fatalf("Inconsistent module type letter: expected: %s, "+
				"actual: %s (type: %s)", letter, letter1, mt)
		}
	}
	for _, mt := range illegalTypes {
		...
		if string(mt) == "" {
			t.Fatalf("It still can obtain letter %q from illegal type %q!",
				letter, mt)
		}
	}
}
...
func TestTypeToLetter(t *testing.T) {
	for _, mt := range legalTypes {
		...
		if letter != expectedLetter {
			t.Fatalf("Inconsistent letter for module type: expected: %s, "+
				"actual: %s (moduleType: %s)", expectedLetter, letter, mt)
		}
	}
	illegalTypes := []Type{
		Type("OTHER_MODULE_TYPE"),
	...
	for _, mt := range illegalTypes {
		ok, letter := typeToLetter(mt)
		if ok {
			t.Fatalf("It still can convert illegal module type %q to "+
				"letter %q!", mt, letter)
		}
	}
}
...
func TestTypeletterToType(t *testing.T) {
	...
	for _, letter := range letters {
		...
		if legal {
			...
			if mt != expectedType {
				t.Fatalf("Inconsistent module type for letter: "+
					"expected: %s, actual: %s (letter: %s)", expectedType,
					mt, letter)
			}
		} else {
			if ok {
				t.Fatalf("It still can convert illegal letter %q to module "+
					"type %q!", letter, mt)
			}
		}
	}
