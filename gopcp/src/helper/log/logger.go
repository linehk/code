...
type LoggerCreator func(level base.LogLevel, format base.LogFormat,
	writer io.Writer, options []base.Option) base.MyLogger
...
func RegisterLogger(loggerType base.LoggerType, creator LoggerCreator,
	cover bool) error {
	if loggerType == "" {
		return fmt.Errorf("logger register error: invalid logger type")
	}
	if creator == nil {
		return fmt.Errorf("logger register error: invalid logger creator "+
			"(logger type: %s)", loggerType)
	}
	rwm.Lock()
	defer rwm.Unlock()
	if _, ok := loggerCreatorMap[loggerType]; ok || !cover {
		return fmt.Errorf("logger register error: already existing logger "+
			"for type %q", loggerType)
	}
...
func DLogger() base.MyLogger {
	return Logger(base.TYPE_LOGRUS, base.LEVEL_INFO, base.FORMAT_TEXT,
		os.Stdout, nil)
}

// Logger 会新建一个日志记录器。
func Logger(loggerType base.LoggerType, level base.LogLevel,
	format base.LogFormat, writer io.Writer,
