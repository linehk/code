...
func main() {
	...
	if len(inputPath) == 0 {
		...
		log.Printf("WARNING: Not specified the flag named input, use "+
			"current path '%s'.", currentPath)
	} else {
		if !isDir(inputPath) {
			log.Fatalf("ERROR: The input path '%s' is not a directory!",
				inputPath)
		}
	}
	if len(outputPath) == 0 {
		outputPath = currentPath
		log.Printf("WARNING: Not specified the flag named output, use "+
			"current path '%s'.", currentPath)
	} else {
		if !isDir(outputPath) {
			log.Fatalf("ERROR: The output path '%s' is not a directory!",
				outputPath)
		}
	}
...
func findFieldTypePrefixes(filePath string) ([]string, error) {
	astFile, err := parser.ParseFile(token.NewFileSet(), filePath, nil,
		parser.AllErrors)
	if err != nil {
		return nil, err
	}
		...
		if genDecl, ok := decl.(*ast.GenDecl); ok &&
			...
			for _, spec := range genDecl.Specs {
				if valueSpec, ok := spec.(*ast.ValueSpec); ok {
					...
					if valueSpec.Type.(*ast.Ident).Name == "FieldType" &&
						name != "UnknownType" {
...
// generate 会生成源码内容。
func (g *Generator) generate(pkgName string,
	prefixes ...string) ([]byte, error) {
	var content []byte
	g.genHeader(pkgName)
...
func (g *Generator) genFieldDecls(prefixes ...string) error {
	...
	t := template.Must(template.New("xfield").Funcs(funcMap).
		Parse(field_decl_template))
...
func (g *Generator) format() ([]byte, error) {
	...
	if err != nil {
		log.Printf("WARNING: Cannot format the generated Go source, "+
			"please build it for the detail, error: %s", err)
