	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("Key=%s,Element=%#v", tc.key, tc.element),
			func(t *testing.T) {
				...
				if err != nil {
					t.Fatalf("An error occurs when new a pair: %s (key: "+
						"%s, element: %#v", err, tc.key, tc.element)
				}
				if p == nil {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("Key=%s,Element=%#v", tc.key, tc.element),
			func(t *testing.T) {
				p, err := newPair(tc.key, tc.element)
				if err != nil {
					t.Fatalf("An error occurs when new a pair: %s "+
						"(key: %s, element: %#v)", err, tc.key, tc.element)
				}
				if p.Key() != tc.key {
...
func TestPairNext(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if err != nil {
			t.Fatalf("An error occurs when new a pair: %s (key: %s, "+
				"element: %#v)", err, tc.key, tc.element)
		}
		if prev != nil {
	...
	for i := number - 1; i >= 0; i-- {
		...
		if i == 0 {
			...
			if next.Key() != expectedNext.key {
				t.Fatalf("Inconsistent next key: expected: %s, actual: %s,"+
					"index: %d", expectedNext.key, next.Key(), i)
			}
			if next.Element() != expectedNext.element {
				t.Fatalf("Inconsistent element: expected: %#v, actual: "+
					"%#v, index: %d", expectedNext.element, next.Element(), i)
			}
		}
		current = next
	}
		...
		t.Run(fmt.Sprintf("Key=%s,Element=%#v", tc.key, tc.element),
			func(t *testing.T) {
				...
				if pCopy.Element() != p.Element() {
					t.Fatalf("Inconsistent element: expected: %#v, "+
						"actual: %#v", p.Element(), pCopy.Element())
				}
			})
	}
}
