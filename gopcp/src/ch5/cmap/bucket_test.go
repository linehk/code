...
func TestBucketPut(t *testing.T) {
	...
	for _, p := range testCases {
		ok, err := b.Put(p, nil)
		if err != nil {
			t.Fatalf("An error occurs when putting a pair to the bucket: "+
				"%s (pair: %#v)", err, p)
		}
		if !ok {
		...
		count++
		if b.Size() != count {
			t.Fatalf("Inconsistent size: expected: %d, actual: %d", count,
				b.Size())
		}
	}
...
func TestBucketPutInParallel(t *testing.T) {
	...
	testingFunc := func(p Pair, t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			...
			ok, err = b.Put(p, lock)
			if err != nil {
				t.Fatalf("An error occurs when putting a repeated pair "+
					"to the bucket: %s (pair: %#v)", err, p)
			}
			if ok {
				t.Fatalf("Couldn't put a repeated pair to the bucket! (pair"+
					": %#v)", p)
			}
		}
	}
...
func TestBucketDeleteInParallel(t *testing.T) {
	...
	testingFunc := func(p Pair, t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			...
			if done {
				t.Fatalf("Couldn't delete a pair from bucket again! (pair: "+
					"%#v", p)
			}
		}
	}
	...
	t.Run("All in parallel", func(t *testing.T) {
		t.Run("Put1", func(t *testing.T) {
			...
			for _, p := range testCases1 {
				...
				ok, err := b.Put(p, lock)
				if !ok {
					t.Fatalf("Couldn't put a pair to the bucket! (pair: %#v)",
						p)
				}
				if err != nil {
					t.Fatalf("An error occurs when putting a pair to the "+
		...
		t.Run("Put2", func(t *testing.T) {
			...
			for _, p := range testCases2 {
				...
				ok, err := b.Put(p, lock)
				if !ok {
					t.Fatalf("Couldn't put a pair to the bucket! "+
						"(pair: %#v)", p)
				}
				if err != nil {
					t.Fatalf("An error occurs when putting a pair to the "+
