...
// TCPComm 表示 TCP 通讯器的结构。
type TCPComm struct {
	addr string
}

// NewTCPComm 会新建一个 TCP 通讯器。
...
// Call 会发起一次通讯。
func (comm *TCPComm) Call(req []byte,
	timeoutNS time.Duration) ([]byte, error) {
	conn, err := net.DialTimeout("tcp", comm.addr, timeoutNS)
	if err != nil {
		return nil, err
	}
	_, err = write(conn, req, DELTM)
	if err != nil {
		return nil, err
	}
	return read(conn, DELTM)
}

// CheckResp 会检查响应内容。
func (comm *TCPComm) CheckResp(rawReq loadgenlib.RawReq,
