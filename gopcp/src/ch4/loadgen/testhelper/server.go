...
func genFormula(operands []int, operator string, result int,
	equal bool) string {
...
// reqHandler 会把参数 sresp 代表的请求转换为数据并发送给连接。
func reqHandler(conn net.Conn) {
	var errMsg string
	var sresp ServerResp
	req, err := read(conn, DELTM)
	if err != nil {
		...
		if err != nil {
			...
			sresp.Formula = genFormula(sreq.Operands, sreq.Operator,
				sresp.Result, true)
		}
	}
	if errMsg != "" {
	...
	_, err = write(conn, bytes, DELTM)
	if err != nil {
		logger.Errorf("Server: Resp Write error: %s", err)
	}
}

// TCPServer 表示基于 TCP 协议的服务器。
type TCPServer struct {
	listener net.Listener
	active   uint32 // 0-未激活；1-已激活。
}

// NewTCPServer 会新建一个基于 TCP 协议的服务器。
		...
		for {
			...
			if err != nil {
				if atomic.LoadUint32(&server.active) == 1 {
					...
					logger.Errorf("Server: Broken acception because of " +
