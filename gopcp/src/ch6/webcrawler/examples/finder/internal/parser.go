...
// genResponseParsers 用于生成响应解析器。
func genResponseParsers() []module.ParseResponse {
	parseLink := func(httpResp *http.Response,
		...
		doc.Find("a").Each(func(index int, sel *goquery.Selection) {
			...
			if err != nil {
				logger.Warnf("An error occurs when parsing "+
					"attribute %q in tag %q : %s (href: %s)", err, "href", "a",
					href)
				return
			}
			if !aURL.IsAbs() {
	...
	parseImg := func(httpResp *http.Response,
		respDepth uint32) ([]module.Data, []error) {
