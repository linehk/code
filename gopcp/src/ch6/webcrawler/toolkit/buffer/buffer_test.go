...
func TestBufferNew(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("An error occurs when new a buffer: %s (size: %d)", err, size)
	}
	if buf == nil {
...
func TestBufferPut(t *testing.T) {
	...
	for _, datum = range data {
		...
		if err != nil {
			t.Fatalf("An error occurs when putting a datum to the buffer: %s "+
				"datum: %d)", err, datum)
		}
		if !ok {
			t.Fatalf("Couldn't put datum to the buffer! (datum: %d)", datum)
		}
		count++
		if buf.Len() != count {
...
func TestBufferGet(t *testing.T) {
	...
	for i := uint32(0); i < size; i++ {
		d, err := buf.Get()
		if err != nil {
			t.Fatalf("An error occurs when getting a datum from the buffer"+
				": %s", err)
		}
		datum, ok = d.(uint32)
		if !ok {
		...
		if datum != i {
			t.Fatalf("Inconsistent datum: expected: %#v, actual: %#v", i, datum)
		}
		count--
		if buf.Len() != count {
			t.Fatalf("Inconsistent buffer len: expected: %d, actual: %d",
	...
	t.Run("All in parallel", func(t *testing.T) {
		t.Run("Put1", func(t *testing.T) {
			...
			for i := begin; i < end; i++ {
				...
				if err != nil {
					t.Fatalf("An error occurs when putting a datum to the "+
						"buffer: %s (datum: %d)", err, i)
				}
				if !ok && atomic.LoadUint32(&gettingCount) == 0 &&
					buf.Len() < buf.Cap() {
					t.Fatalf("Couldn't put datum to the buffer! (datum: %d)", i)
				}
		...
		t.Run("Put2", func(t *testing.T) {
			...
			for i := begin; i < end; i++ {
				...
				if !ok && atomic.LoadUint32(&gettingCount) == 0 &&
					buf.Len() < buf.Cap() {
					t.Fatalf("Couldn't put datum to the buffer! "+
						"(datum: %d)", i)
				}
				atomic.AddUint32(&puttingCount, ^uint32(0))
			}
		...
		t.Run("Get2", func(t *testing.T) {
			...
			for i := uint32(0); i < max; i++ {
				d, err := buf.Get()
				if err != nil {
					t.Fatalf("An error occurs when getting a datum from "+
						"the buffer: %s", err)
				}
				if d == nil && atomic.LoadUint32(&puttingCount) == 0 &&
					buf.Len() != 0 {
...
func TestBufferCloseInParallel(t *testing.T) {
	...
	t.Run("Put", func(t *testing.T) {
		...
		for i := uint32(0); i < maxNumber; i++ {
			...
			if err == nil && buf.Closed() {
				t.Fatalf("It still can put datum to the closed buffer! "+
					"datum: %d)", i)
			}
		}
	})
		...
		for i := uint32(0); i < max; i++ {
			...
			if buf.Closed() {
				if _, err = buf.Get(); err == nil {
					t.Fatalf("It still can get datum from the closed "+
						"buffer! (datum: %d)", i)
				}
			}
		}
	...
	t.Run("Close", func(t *testing.T) {
		...
		if !buf.Closed() {
			t.Fatalf("Inconsistent buffer status: expected closed: %v, "+
				"actual closed: %v", true, buf.Closed())
		}
		ok = buf.Close()
		if ok {
