...
func TestMyPublicSuffixList(t *testing.T) {
	...
	for i := 0; i < domainLen; i++ {
		...
		if suffix != expectedPS {
			t.Fatalf("Inconsistent publice suffix for domain %q: expected: "+
				"%s, actual: %s", domain, expectedPS, suffix)
		}
	}
	expectedString := "Web crawler - public suffix list (rev 1.0) " +
		"power by \"golang.org/x/net/publicsuffix\""
	actualString := psl.String()
	if actualString != expectedString {
		t.Fatalf("Inconsistent string for myPublicSuffixList: expected: %s, "+
