...
func TestSchedInit(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("No error when initialize scheduler with "+
			"illegal request arguments %v!", invalidRequestArgs)
	}
	...
	if err == nil {
		t.Fatalf("No error when initialize scheduler with illegal "+
			"data arguments %v!", invalidDataArgs)
	}
	...
	for _, invalidModuleArgs := range invalidModuleArgsList {
		...
		if err == nil {
			t.Fatalf("No error when initialize scheduler with illegal "+
				"module arguments %s!", invalidModuleArgs)
		}
	}
	invalidModuleArgsList = []ModuleArgs{
...
func TestSchedStop(t *testing.T) {
	...
	firstHTTPReq, err := http.NewRequest("GET", url, nil)
	if err != nil {
		t.Fatalf("An error occurs when creating a HTTP "+
			"request: %s (url: %s)", err, url)
	}
	err = sched.Start(firstHTTPReq)
	if err != nil {
		t.Fatalf("An error occurs when starting scheduler: %s", err)
...
func TestSchedStatus(t *testing.T) {
	...
	// 测试重复初始化。
	if err = sched.Init(requestArgs, dataArgs, moduleArgs); err != nil {
		t.Fatalf("An error occurs when repeatedly initializing scheduler: %s",
			err)
	}
	...
	// 测试已初始化状态下的启动。
	if err = sched.Start(firstHTTPReq); err != nil {
		t.Fatalf("An error occurs when starting scheduler after initialize: %s",
			err)
	}
	...
	// 测试已停止状态下的初始化。
	if err = sched.Init(requestArgs, dataArgs, moduleArgs); err != nil {
		t.Fatalf("An error occurs when initializing scheduler after stop: %s",
			err)
	}
}
...
func TestSchedSendReq(t *testing.T) {
	...
	if urlMapLen != 1 {
		t.Fatalf("Inconsistent URL map length: expected: %d, actual: %d", 1,
			urlMapLen)
	}
