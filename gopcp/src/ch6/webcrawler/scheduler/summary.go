...
func newSchedSummary(requestArgs RequestArgs, dataArgs DataArgs,
	moduleArgs ModuleArgs, sched *myScheduler) SchedSummary {
	if sched == nil {
		return nil
	}
...
func (one *SummaryStruct) Same(another SummaryStruct) bool {
	...
	if another.Downloaders == nil || len(another.Downloaders) !=
		len(one.Downloaders) {
		return false
	}
	...
	if another.Analyzers == nil || len(another.Analyzers) !=
		len(one.Analyzers) {
		return false
	}
	...
	if another.Pipelines == nil || len(another.Pipelines) !=
		len(one.Pipelines) {
		return false
	}
...
func getModuleSummaries(registrar module.Registrar,
	mType module.Type) []module.SummaryStruct {
