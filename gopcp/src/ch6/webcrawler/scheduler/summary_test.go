...
func TestSummaryNew(t *testing.T) {
	...
	summary := newSchedSummary(requestArgs, dataArgs, moduleArgs,
		sched.(*myScheduler))
	if summary == nil {
...
func TestSummaryStruct(t *testing.T) {
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different module " +
			"arguments summary!")
	}
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different analyzers " +
			"summary number!")
	}
	another.Analyzers = make([]module.SummaryStruct, len(one.Analyzers))
	copy(another.Analyzers, one.Analyzers)
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different item " +
			"pipelines summary!")
	}
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different item pipelines " +
			"summary number!")
	}
	another.Pipelines = make([]module.SummaryStruct, len(one.Pipelines))
	copy(another.Pipelines, one.Pipelines)
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different request " +
			"buffer summary!")
	}
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different response " +
			"buffer summary!")
	}
	...
	if one.Same(another) {
		t.Fatalf("Same scheduler summaries with different error " +
			"buffer summary!")
	}
...
func TestSummaryString(t *testing.T) {
	...
	if summaryStr != expectedSummaryStr {
		t.Fatalf("Inconsistent scheduler summary: expected:\n%s\nactual:\n%s",
