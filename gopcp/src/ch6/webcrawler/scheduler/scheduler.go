...
type Scheduler interface {
	...
	// 参数 firstHTTPReq 代表首次请求。调度器会以此为起始点开始执行爬取流程。
...
func (sched *myScheduler) Init(requestArgs RequestArgs, dataArgs DataArgs,
	moduleArgs ModuleArgs) (err error) {
	// 检查状态。
	...
	logger.Info("-- Accepted primary domains: %v", requestArgs.AcceptedDomains)
	sched.urlMap, _ = cmap.NewConcurrentMap(16, nil)
	logger.Infof("-- URL map: length: %d, concurrency: %d", sched.urlMap.Len(),
		sched.urlMap.Concurrency())
...
func (sched *myScheduler) ErrorChan() <-chan error {
	...
	go func(errBuffer buffer.Pool, errCh chan error) {
		for {
			...
			if err != nil {
				logger.Warnln("The error buffer pool was closed. " +
					"Break error reception.")
				close(errCh)
				break
			}
			err, ok := datum.(error)
...
func (sched *myScheduler) registerModules(moduleArgs ModuleArgs) error {
	for _, d := range moduleArgs.Downloaders {
		...
		if !ok {
			errMsg := fmt.Sprintf("Couldn't register downloader instance "+
				"with MID %q!", d.ID())
			return genError(errMsg)
		}
	}
	logger.Infof("All downloaders have been registered. (number: %d)",
	...
	for _, a := range moduleArgs.Analyzers {
		...
		if !ok {
			errMsg := fmt.Sprintf("Couldn't register analyzer instance "+
				"with MID %q!", a.ID())
			return genError(errMsg)
		}
	}
	...
	for _, p := range moduleArgs.Pipelines {
		...
		if !ok {
			errMsg := fmt.Sprintf("Couldn't register pipeline instance "+
				"with MID %q!", p.ID())
			return genError(errMsg)
		}
	}
...
func (sched *myScheduler) download() {
	go func() {
		for {
			...
			if err != nil {
				logger.Warnln("The request buffer pool was closed. " +
					"Break request reception.")
				break
			}
...
func (sched *myScheduler) analyze() {
	go func() {
		for {
			...
			if err != nil {
				logger.Warnln("The response buffer pool was closed. " +
					"Break response reception.")
				break
			}
...
func (sched *myScheduler) analyzeOne(resp *module.Response) {
	...
	if dataList != nil {
		for _, data := range dataList {
			...
			default:
				errMsg := fmt.Sprintf("Unsupported data type %T! (data: %#v)",
					d, d)
				sendError(errors.New(errMsg), m.ID(), sched.errorBufferPool)
			}
		}
...
func (sched *myScheduler) pick() {
	go func() {
		for {
			...
			if err != nil {
				logger.Warnln("The item buffer pool was closed. " +
					"Break item reception.")
				break
			}
...
func (sched *myScheduler) sendReq(req *module.Request) bool {
	...
	if scheme != "http" && scheme != "https" {
		logger.Warnf("Ignore the request! Its URL scheme is %q, but "+
			"should be %q or %q. (URL: %s)\n", scheme, "http", "https", reqURL)
		return false
	}
	...
	if sched.acceptedDomainMap.Get(pd) == nil {
		...
		logger.Warnf("Ignore the request! Its host %q is not accepted "+
			"primary domain map. (URL: %s)\n", httpReq.Host, reqURL)
		return false
	}
	if req.Depth() > sched.maxDepth {
		logger.Warnf("Ignore the request! Its depth %d is greater "+
...
func sendItem(item module.Item, itemBufferPool buffer.Pool) bool {
	...
	go func(item module.Item) {
		if err := itemBufferPool.Put(item); err != nil {
			logger.Warnln("The item buffer pool was closed. Ignore " +
				"item sending.")
		}
	}(item)
	return true
...
func (sched *myScheduler) initBufferPool(dataArgs DataArgs) {
	...
	logger.Infof("-- Response buffer pool: bufferCap: %d, maxBufferNumber: %d",
		sched.respBufferPool.BufferCap(),
		sched.respBufferPool.MaxBufferNumber())
	...
	logger.Infof("-- Item buffer pool: bufferCap: %d, maxBufferNumber: %d",
		sched.itemBufferPool.BufferCap(),
		sched.itemBufferPool.MaxBufferNumber())

	// 初始化错误缓冲池。
	if sched.errorBufferPool != nil && !sched.errorBufferPool.Closed() {
		sched.errorBufferPool.Close()
	}
	sched.errorBufferPool, _ = buffer.NewPool(dataArgs.ErrorBufferCap,
		dataArgs.ErrorMaxBufferNumber)
	logger.Infof("-- Error buffer pool: bufferCap: %d, maxBufferNumber: %d",
		sched.errorBufferPool.BufferCap(),
		sched.errorBufferPool.MaxBufferNumber())
}
