...
func TestCheckStatus(t *testing.T) {
	...
	for _, currentStatus := range currentStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("It still can check status with incorrect current "+
				"status %q!", GetStatusDescription(currentStatus))
		}
	}
	...
	for _, wantedStatus := range wantedStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("It still can check status with incorrect wanted "+
				"status %q", GetStatusDescription(wantedStatus))
		}
	}
	...
	for _, wantedStatus := range wantedStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("It still can check status with current status %q wanted "+
				"status %q!", GetStatusDescription(currentStatus),
	...
	if err := checkStatus(currentStatus, wantedStatus, nil); err != nil {
		t.Fatalf("An error occurs when checking status: %s "+
			"(currentStatus: %q, wantedStatus: %q)!", err,
	...
	for _, wantedStatus := range wantedStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("It still can check status with current status %q "+
				"wanted status %q!", GetStatusDescription(currentStatus),
				GetStatusDescription(wantedStatus))
		}
	}
...
func TestGetStatusDescription(t *testing.T) {
	...
	for status, expectedDesc := range statusMap {
		...
		if desc != expectedDesc {
			t.Fatalf("Inconsistent description for status %d: expected: %s, "+
				"actual: %s", status, expectedDesc, desc)
		}
	}
}
