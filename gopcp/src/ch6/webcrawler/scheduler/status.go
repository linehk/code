...
func checkStatus(currentStatus Status, wantedStatus Status,
	...
	default:
		errMsg :=
			fmt.Sprintf("unsupported wanted status for check! "+
				"(wantedStatus: %d)", wantedStatus)
		err = genError(errMsg)
	}
	return
}
