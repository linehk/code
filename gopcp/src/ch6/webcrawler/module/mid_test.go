...
func TestMIDGenAndSplit(t *testing.T) {
	...
	for _, addr := range addrs {
		for _, mt := range legalTypes {
			...
			if err != nil {
				t.Fatalf("An error occurs when generating module ID: %s "+
					"(type: %s, sn: %d, addr: %s)", err, mt, expectedSN, addr)
			}
			...
			if err != nil {
				t.Fatalf("An error occurs when splitting MIDd %q: %s", mid, err)
			}
			letter, snStr, addrStr := parts[0], parts[1], parts[2]
			if letter != expectedLetter {
				t.Fatalf("Inconsistent type letter in MID: expected: %s, "+
					"actual: %s", expectedLetter, letter)
			}
			sn, err := strconv.ParseUint(snStr, 10, 64)
			if err != nil {
				t.Fatalf("An error occurs when parsing SN: %s (snStr: %s)",
					err, snStr)
			}
			if sn != expectedSN {
			...
			if addrStr != expectedAddrStr {
				t.Fatalf("Inconsistent address string in MID: expected: %s, "+
					"actual: %s", expectedAddrStr, addrStr)
			}
		}
	}
	for _, addr := range addrs {
		for _, mt := range illegalTypes {
			...
			if err == nil {
				t.Fatalf("It still can generate module ID with illegal "+
					"type %q!", mt)
			}
			if string(mid) != "" {
				t.Fatalf("It still can generate module ID %q with illegal "+
					"type %q!", mid, mt)
			}
		}
	}
...
func TestMIDLegal(t *testing.T) {
	...
	for _, mt := range legalTypes {
		for mip := range legalIPMap {
			...
			if err != nil {
				t.Fatalf("An error occurs when judging the legality for "+
					"MID: %s (type: %s, sn: %d, addr: %s)", err, mt, sn, addr)
			}
			if !LegalMID(mid) {
				t.Fatalf("The generated MID %q is legal, but do not be "+
					"detected!", mid)
			}
		}
	}
	for _, illegalMID := range illegalMIDs {
		if LegalMID(illegalMID) {
			t.Fatalf("The generated MID %q is illegal, but do not be detected!",
				illegalMID)
		}
	}
}
