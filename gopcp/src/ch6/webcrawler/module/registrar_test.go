...
func TestRegRegister(t *testing.T) {
	...
	for _, mt := range legalTypes {
		...
		for mip := range legalIPMap {
			...
			ok, err = registrar.Register(m)
			if err != nil {
				t.Fatalf("An error occurs when registering module instance: "+
					"%s (MID: %s)", err, mid)
			}
			if !ok {
			...
			if ok {
				t.Fatalf("It still can repeatedly register module instance "+
					"with same MID %q!", mid)
			}
			...
			ok, err = registrar.Register(m)
			if err == nil {
				t.Fatalf("No error when register module instance with "+
					"illegal MID %q!", mid)
			}
			if ok {
				t.Fatalf("It still can repeatedly register module "+
					"instance! (MID: %s)", mid)
			}
		}
		modules, err := registrar.GetAllByType(mt)
		if err != nil {
			t.Fatalf("An error occurs when getting all module instances: %s "+
				"(type: %s)", err, mt)
		}
	...
	for _, mt := range illegalTypes {
		...
		ok, err := registrar.Register(m)
		if err == nil {
			t.Fatalf("No error when resigter module instance with illegal "+
				"type %q!", mt)
		}
		if ok {
			t.Fatalf("It still can register module instance with illegal "+
				"type %q!", mt)
		}
	}
}
...
func TestModuleUnregister(t *testing.T) {
	...
	for _, mt := range legalTypes {
		for mip := range legalIPMap {
			...
			if err != nil {
				t.Fatalf("An error occurs when generating module "+
					"ID: %s (type: %s, sn: %d, addr: %s)", err, mt, sn, addr)
			}
			m := fakeModuleFuncMap[mt](mid)
			_, err = registrar.Register(m)
			if err != nil {
				t.Fatalf("An error occurs when registering module "+
					"instance: %s (type: %s, sn: %d, addr: %s)", err, mt, sn,
					addr)
			}
	...
	for _, mid := range mids {
		...
		if err != nil {
			t.Fatalf("An error occurs when unregistering module "+
				"instance: %s (mid: %s)", err, mid)
		}
		if !ok {
			t.Fatalf("Couldn't unregister module instance! (MID: %s)", mid)
		}
	}
	...
	for _, mid := range mids {
		...
		if ok {
			t.Fatalf("It still can unregister nonexistent module "+
				"instance! (mid: %s)", mid)
		}
	}
	for _, illegalMID := range illegalMIDs {
		...
		if err == nil {
			t.Fatalf("No error when unregister module instance with illegal "+
				"MID %q!", illegalMID)
		}
		if ok {
			t.Fatalf("It still can unregister module instance with "+
				"illegal MID %q", illegalMID)
		}
	}
}
...
func TestModuleGet(t *testing.T) {
	...
	addr, _ := NewAddr("http", "127.0.0.1", 8080)
	mid := MID(fmt.Sprintf(midTemplate, legalTypeLetterMap[mt],
		DefaultSNGen.Get(), addr))
	...
	t.Run("All in parallel", func(t *testing.T) {
		t.Run("Register", func(t *testing.T) {
			...
			for i, addr := range addrs {
				...
				if err != nil {
					t.Fatalf("An error occurs when generating module "+
						"ID: %s (type: %s, sn: %d, addr: %s)", err, mt, sn,
						addr)
				}
				m := fakeModuleFuncMap[mt](mid)
				_, err = registrar.Register(m)
				if err != nil {
					t.Fatalf("An error occurs when registering module "+
						"instance: %s (type: %s, sn: %d, addr: %s)", err,
						mt, sn, addr)
				}
			}
		})
		...
		t.Run("Get", func(t *testing.T) {
			...
			for _, mt := range types {
				...
				if err != nil && err != ErrNotFoundModuleInstance {
					t.Fatalf("An error occurs when getting module "+
						"instance: %s (mid: %s)", err, m.ID())
				}
			}
		})
	})
