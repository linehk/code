...
func TestNew(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("An error occurs when creating a pipeline: %s (mid: %s, "+
			"processors: %#v)", err, mid, processors)
	}
	if p == nil {
	...
	if len(p.ItemProcessors()) != len(processors) {
		t.Fatalf("Inconsistent item processor number for pipeline: "+
			"expected: %d, actual: %d", len(p.ItemProcessors()), len(processors))
	}
...
func TestSend(t *testing.T) {
	...
	errs := p.Send(nil)
	if len(errs) != 1 {
		t.Fatalf("Inconsistent error number after Send(): expected: "+
			"%d, actual: %d", 1, len(errs))
	}
	item := module.Item(map[string]interface{}{"number": 0})
	errs = p.Send(item)
	number := item["number"].(int)
	if number != processorNumber {
		t.Fatalf("Inconsistent number in item after Send(): expected: "+
			"%d, actual: %d", processorNumber, number)
	}
	if len(errs) != expectedErrs {
		t.Fatalf("Inconsistent error number after Send(): expected: "+
			"%d, actual: %d", expectedErrs, len(errs))
	}
...
func TestFailFast(t *testing.T) {
	...
	if p.FailFast() != false {
		t.Fatalf("Inconsistent fail fast sign for pipeline: expected: "+
			"%v, actual: %v", false, p.FailFast())
	}
	p.SetFailFast(true)
	if p.FailFast() != true {
		t.Fatalf("Inconsistent fail fast sign for pipeline: expected: "+
			"%v, actual: %v", true, p.FailFast())
	}
}
...
func TestCount(t *testing.T) {
	...
	if pi.CalledCount() != 1 {
		t.Fatalf("Inconsistent called count for internal module: "+
			"expected: %d, actual: %d", 1, pi.CalledCount())
	}
	if pi.AcceptedCount() != 1 {
		t.Fatalf("Inconsistent accepted count for internal module: "+
			"expected: %d, actual: %d", 1, pi.AcceptedCount())
	}
	if pi.CompletedCount() != 0 {
		t.Fatalf("Inconsistent completed count for internal module: "+
			"expected: %d, actual: %d", 0, pi.CompletedCount())
	...
	if pi.CompletedCount() != 1 {
		t.Fatalf("Inconsistent completed count for internal module: "+
			"expected: %d, actual: %d", 1, pi.CompletedCount())
	}
	if pi.HandlingNumber() != 0 {
		t.Fatalf("Inconsistent handling number for internal module: "+
			"expected: %d, actual: %d", 0, pi.HandlingNumber())
