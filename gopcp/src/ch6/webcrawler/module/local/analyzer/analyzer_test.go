...
// testingReader 代表测试专用的读取器，实现了 io.ReadCloser 接口类型。
...
func TestNew(t *testing.T) {
	...
	if len(a.RespParsers()) != len(parsers) {
		t.Fatalf("Inconsistent response parser number for pipeline: "+
			"expected: %d, actual: %d", len(a.RespParsers()), len(parsers))
	}
	...
	for _, parsers := range parsersList {
		a, err = New(mid, parsers, nil)
		if err == nil {
			t.Fatalf("No error when create an analyzer with illegal "+
				"parsers %#v!", parsers)
		}
	}
}
...
func TestAnalyze(t *testing.T) {
	...
	for i, d := range data {
		...
		if !ok {
			t.Errorf("Inconsistent datum type: expected: %T, actual: %T "+
				"(index: %d)", module.Item{}, d, i)
		}
		...
		if !ok {
			t.Errorf("Inconsistent index type: expected: %T, actual: %T "+
				"(index: %d)", int(0), item["index"], i)
		}
		if index != count {
			t.Errorf("Inconsistent index: expected: %d, actual: %d "+
				"(index: %d)", count, index, i)
		}
		depth, ok := item["depth"].(uint32)
		if !ok {
			t.Errorf("Inconsistent depth type: expected: %T, actual: %T "+
				"(index: %d)", uint32(0), item["depth"], i)
		}
...
func TestCount(t *testing.T) {
	...
	if ai.CalledCount() != 0 {
		t.Fatalf("Inconsistent called count for internal module: expected: "+
			"%d, actual: %d", 0, ai.CalledCount())
	}
	if ai.AcceptedCount() != 0 {
		t.Fatalf("Inconsistent accepted count for internal module: expected: "+
			"%d, actual: %d", 0, ai.AcceptedCount())
	}
	if ai.CompletedCount() != 0 {
		t.Fatalf("Inconsistent completed count for internal module: expected: "+
			"%d, actual: %d", 0, ai.CompletedCount())
	}
	if ai.HandlingNumber() != 0 {
		t.Fatalf("Inconsistent handling number for internal module: expected: "+
			"%d, actual: %d", 0, ai.HandlingNumber())
	}
	...
	if ai.CalledCount() != 1 {
		t.Fatalf("Inconsistent called count for internal module: expected: "+
			"%d, actual: %d", 1, ai.CalledCount())
	}
	if ai.AcceptedCount() != 1 {
		t.Fatalf("Inconsistent accepted count for internal module: expected: "+
			"%d, actual: %d", 1, ai.AcceptedCount())
	}
	if ai.CompletedCount() != 0 {
		t.Fatalf("Inconsistent completed count for internal module: expected: "+
			"%d, actual: %d", 0, ai.CompletedCount())
	...
	if ai.CompletedCount() != 1 {
		t.Fatalf("Inconsistent completed count for internal module: expected: "+
			"%d, actual: %d", 1, ai.CompletedCount())
	}
	if ai.HandlingNumber() != 0 {
		t.Fatalf("Inconsistent handling number for internal module: expected: "+
			"%d, actual: %d", 0, ai.HandlingNumber())
...
func genTestingRespParser(fail bool) module.ParseResponse {
	if fail {
		return func(httpResp *http.Response,
			...
			errs :=
				[]error{fmt.Errorf("Fail! (httpReqp: %#v, respDepth: %#v)",
					httpResp, respDepth)}
			return nil, errs
		}
	}
...
func getTestingResps(number uint32, method string, url string, depth uint32,
	t *testing.T) []*module.Response {
	httpReq, err := http.NewRequest(method, url, nil)
	if err != nil {
		t.Fatalf("An error occurs when creating a HTTP request: %s "+
