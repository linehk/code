...
var ( // profile flags
	...
	memProfileRate = flag.Int("memprofilerate", 0,
		"if > 0, sets runtime.MemProfileRate")
	cpuProfile = flag.String("cpuprofile", "",
		"write a cpu profile to the named file during execution")
	blockProfile = flag.String("blockprofile", "",
		"write a goroutine blocking profile to the named file after execution")
	blockProfileRate = flag.Int("blockprofilerate", 1,
...
func startCPUProfile() {
	if *cpuProfile != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Couldn't create cpu profile "+
				"output file: %s\n", err)
			return
		}
...
func stopBlockProfile() {
	if *blockProfile != "" && *blockProfileRate >= 0 {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Couldn't create block profile output "+
				"file: %s\n", err)
			return
		}
...
func stopCPUProfile() {
	if *cpuProfile != "" {
		pprof.StopCPUProfile() // 把记录的概要信息写道已指定的文件
	}
}
...
func stopMemProfile() {
	if *memProfile != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Couldn't create mem profile output "+
				"file: %s\n", err)
			return
		}
...
func SaveProfile(workDir string, profileName string,
	ptype ProfileType, debug int) {
