...
func serverGo() {
	...
	printServerLog("Got listener for the server. (local address: %s)",
		listener.Addr())
	for {
...
func handleConn(conn net.Conn) {
	...
	for {
		...
		if err != nil {
			...
			printServerLog("Sent error message (written %d bytes): %s.",
				n, err)
			continue
		}
...
func clientGo(id int) {
	...
	defer conn.Close()
	printClientLog(id, "Connected to server. (remote address: %s, "+
		"local address: %s)", conn.RemoteAddr(), conn.LocalAddr())
	...
	for j := 0; j < requestNumber; j++ {
		...
		if err != nil {
			if err == io.EOF {
				printClientLog(id, "The connection is closed by cnother side.")
			} else {
