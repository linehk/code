...
func TestSegmentPut(t *testing.T) {
	...
	for _, p := range testCases {
		ok, err := s.Put(p)
		if err != nil {
			t.Fatalf("An error occurs when putting a pair to the segment: "+
				"%s (pair: %#v)", err, p)
		}
		if !ok {
		...
		if actualPair == nil {
			t.Fatalf("Inconsitent pair: expected: %#v, actual: %#v",
				p.Element(), nil)
		}
		ok, err = s.Put(p)
		if err != nil {
			t.Fatalf("An error occurs when putting a repeated pair to the "+
				"segment: %s (pair: %#v)", err, p)
		}
		if ok {
		...
		count++
		if s.Size() != count {
			t.Fatalf("Inconsitent size: expected: %d, actual: %d", count,
				s.Size())
		}
	}
	if s.Size() != uint64(number) {
		t.Fatalf("Inconsitent size: expected: %d, actual: %d", number, s.Size())
	}
}
...
func TestSegmentPutInParallel(t *testing.T) {
	...
	testingFunc := func(p Pair, t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			...
			ok, err = s.Put(p)
			if err != nil {
				t.Fatalf("An error occurs when putting a repeated pair to "+
					"the segment: %s (pair: %#v)", err, p)
			}
			if ok {
				t.Fatalf("Couldn't put a repeated pair to the segment! "+
					"(pair: %#v)", p)
			}
		}
	}
	...
	testingFunc := func(p Pair, t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			...
			if actualPair.Key() != p.Key() {
				t.Fatalf("Inconsitent key: expected: %s, actual: %s",
					p.Key(), actualPair.Key())
			}
			if actualPair.Hash() != p.Hash() {
				t.Fatalf("Inconsitent hash: expected: %d, actual: %d",
					p.Hash(), actualPair.Hash())
			}
			if actualPair.Element() != p.Element() {
				t.Fatalf("Inconsitent element: expected: %#v, actual: %#v",
...
func TestSegmentDelete(t *testing.T) {
	...
	for _, p := range testCases {
		...
		if done {
			t.Fatalf("Couldn't delete a pair from segment again! (pair: %#v)",
				p)
		}
		if count > 0 {
			count--
	...
	if s.Size() != 0 {
		t.Fatalf("Inconsitent size: expected: %d, actual: %d", 0, s.Size())
	}
}
...
func TestSegmentDeleteInParallel(t *testing.T) {
	...
	testingFunc := func(p Pair, t *testing.T) func(t *testing.T) {
		return func(t *testing.T) {
			...
			if done {
				t.Fatalf("Couldn't delete a pair from segment again! (pair: "+
					"%#v", p)
			}
		}
	}
	...
	t.Run("All in parallel", func(t *testing.T) {
		t.Run("Put1", func(t *testing.T) {
			...
			for _, p := range testCases1 {
				_, err := s.Put(p)
				if err != nil {
					t.Fatalf("An error occurs when putting a pair to the "+
						"segment: %s (pair: %#v)", err, p)
				}
			}
		})
		...
		t.Run("Get1", func(t *testing.T) {
			...
			for _, p := range testCases1 {
				...
				if actualPair.Hash() != p.Hash() {
					t.Fatalf("Inconsitent hash: expected: %d, actual: %d "+
						"(key=%s)", p.Hash(), actualPair.Hash(), p.Key())
				}
				if actualPair.Element() != p.Element() {
					t.Fatalf("Inconsitent element: expected: %#v, actual: "+
						"%#v (key=%s)", p.Element(), actualPair.Element(),
						p.Key())
				}
			}
		})
