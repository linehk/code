...
import (
	...
	sched "ch6/webcrawler/scheduler"
	"helper/log"
)
...
// 参数 summarizeInterval 代表摘要获取间隔时间，单位：纳秒。
// 参数 maxIdleCount 代表最大空闲计数。
// 参数 autoStop 被用来指示该方法是否在调度器空闲足够长的时间之后自行停止调度器。
// 参数 record 代表日志记录函数。
// 当监控结束之后，该方法会向作为唯一结果值的通道发送一个代表了空闲状态检查次数的数值。
func Monitor(scheduler sched.Scheduler, checkInterval time.Duration,
	summarizeInterval time.Duration, maxIdleCount uint,
	...
	logger.Infof("Monitor parameters: checkInterval: %s, "+
		"summarizeInterval: %s, maxIdleCount: %d, autoStop: %v", checkInterval,
		summarizeInterval, maxIdleCount, autoStop)
...
func recordSummary(scheduler sched.Scheduler, summarizeInterval time.Duration,
	record Record, stopNotifier context.Context) {
	go func() {
		// 等待调度器开启。
		waitForSchedulerStart(scheduler)
		...
		for {
			...
			if currNumGoroutine != prevNumGoroutine ||
				...
				if err != nil {
					logger.Errorf("An error occurs when generating "+
						"scheduler summary: %s\n", err)
					continue
				}
...
// reportError 用于接受和报告错误。
func reportError(scheduler sched.Scheduler, record Record,
	...
	go func() {
		...
		for {
			...
			err, ok := <-errorChan
			if ok {
				errMsg := fmt.Sprintf("Received an error from "+
					"error channel: %s", err)
				record(2, errMsg)
			}
