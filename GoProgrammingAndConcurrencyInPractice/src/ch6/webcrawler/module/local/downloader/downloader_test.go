...
func TestNew(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("An error occurs when creating a downloader: %s (mid: %s, "+
			"httpClient: %#v)", err, mid, httpClient)
	}
	if d == nil {
...
func TestDownload(t *testing.T) {
	...
	if resp.Depth() != depth {
		t.Fatalf("Inconsistent depth: expected: %d, actual: %d", depth,
			resp.Depth())
	}
	...
	if err != nil {
		t.Fatalf("An error occurs when reading HTTP response body: %s "+
			"(url: %s)", err, url)
	}
	lineStr := string(line)
	expectedFirstLine := "User-agent: Baiduspider"
	if lineStr != expectedFirstLine {
		t.Fatalf("Inconsistent first line of the HTTP response body: "+
			"expected: %s, actual: %s (url: %s)", expectedFirstLine, lineStr,
			url)
	}
...
func TestCount(t *testing.T) {
	...
	if di.CalledCount() != 1 {
		t.Fatalf("Inconsistent called count for internal module: "+
			"expected: %d, actual: %d", 1, di.CalledCount())
	}
	if di.AcceptedCount() != 1 {
		t.Fatalf("Inconsistent accepted count for internal module: "+
			"expected: %d, actual: %d", 1, di.AcceptedCount())
	}
	if di.CompletedCount() != 0 {
		t.Fatalf("Inconsistent completed count for internal module: "+
			"expected: %d, actual: %d", 0, di.CompletedCount())
	...
	if di.CompletedCount() != 1 {
		t.Fatalf("Inconsistent completed count for internal module: "+
			"expected: %d, actual: %d", 1, di.CompletedCount())
	}
	if di.HandlingNumber() != 0 {
		t.Fatalf("Inconsistent handling number for internal module: "+
			"expected: %d, actual: %d", 0, di.HandlingNumber())
