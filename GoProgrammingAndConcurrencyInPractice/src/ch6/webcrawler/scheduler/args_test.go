...
import (
	...
	"ch6/webcrawler/module/local/analyzer"
	"ch6/webcrawler/module/local/downloader"
	"ch6/webcrawler/module/local/pipeline"
...
func TestArgsRequest(t *testing.T) {
	...
	if !same {
		t.Fatalf("Inconsistent request arguments sameness: expected: %v, "+
			"actual: %v", true, same)
	}

	same = one.Same(nil)
	if same {
		t.Fatalf("Inconsistent request arguments sameness with "+
			"nil parameter: expected: %v, actual: %v", false, same)
	}
	...
	if same {
		t.Fatalf("Inconsistent request arguments sameness with "+
			"different max depth: expected: %v, actual: %v", false, same)
	}
	...
	if same {
		t.Fatalf("Inconsistent request arguments sameness with "+
			"different accepted domains: expected: %v, actual: %v", false, same)
	}
...
func genDataArgs(bufferCap uint32, maxBufferNumber uint32,
	stepLen uint32) DataArgs {
	values := [8]uint32{}
...
func genSimpleModuleArgs(downloaderNumber int8, analyzerNumber int8,
	pipelineNumber int8, t *testing.T) ModuleArgs {
...
func genSimpleDownloaders(number int8, reuseMID bool, snGen module.SNGenertor,
	t *testing.T) []module.Downloader {
	if number < -1 {
		...
		if err != nil {
			t.Fatalf("An error occurs when creating a downloader: %s "+
				"(mid: %s, httpClient: %#v", err, mid, httpClient)
		}
...
func genSimpleAnalyzers(number int8, reuseMID bool, snGen module.SNGenertor,
	t *testing.T) []module.Analyzer {
	...
	if number < -1 {
		...
		if err != nil {
			t.Fatalf("An error occurs when creating an analyzer: %s "+
				"(mid: %s, respParsers: %#v)", err, mid, respParsers)
		}
...
func genSimplePipelines(number int8, reuseMID bool, snGen module.SNGenertor,
	t *testing.T) []module.Pipeline {
	...
	if number < -1 {
		...
		if err != nil {
			t.Fatalf("An error occurs when creating a pipeline: %s "+
				"(mid: %s, processors: %#v)", err, mid, processors)
		}
...
func parseATag(httpResp *http.Response,
	...
	doc.Find("a").Each(func(index int, sel *goquery.Selection) {
		...
		// 暂不支持对 Javascript 代码的解析。
		if href != "" && !strings.HasPrefix(lowerHref, "javascript") {
			aURL, err := url.Parse(href)
			if err != nil {
				logger.Warnf("An error occurs when parsing attribute %q in "+
					"tag %q : %s (href: %s)", err, "href", "a", href)
				return
			}
			if !aURL.IsAbs() {
